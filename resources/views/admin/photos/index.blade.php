...
<div class="container flex flex-row items-center justify-between p-4 mx-auto mt-5 border-b-4">
    <h1 class="text-2xl">All Images</h1>
    <a href="{{ route('admin.photos.create') }}" class="px-6 py-2 text-white bg-gray-700 rounded-lg">
            ...
            <table class="block min-w-full border-collapse md:table">
                <thead class="block md:table-header-group">
                    <tr class="absolute block border border-grey-500 md:border-none md:table-row -top-full md:top-auto -left-full md:left-auto md:relative ">
                        <th class="block p-2 font-bold text-left text-white bg-gray-600 md:border md:border-grey-500 md:table-cell">Post Title</th>
                        <th class="block p-2 font-bold text-left text-white bg-gray-600 md:border md:border-grey-500 md:table-cell"> Photo</th>
                        <th class="block p-2 font-bold text-left text-white bg-gray-600 md:border md:border-grey-500 md:table-cell">Caption</th>
                ...
                <tbody class="block md:table-row-group">
                    @foreach ($photos as $photo)
                        <tr>
                            <td class="block p-2 text-left md:border md:border-grey-500 md:table-cell">{{  $photo->post->title }}</td>
                            <td class="block p-2 text-left md:border md:border-grey-500 md:table-cell">
                                <img src="{{ asset('storage/photos/'.$photo->image) }}" alt="" width="100">
                            </td>
                            <td class="block p-2 text-left md:border md:border-grey-500 md:table-cell">{{  $photo->caption }}</td>

                            <td>
                                <a class="" href="{{ route('admin.photos.edit',$photo) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
